...
export const createCleanupManagement = (
  services: OfflineManagerServices
): CleanupManagement => ({
  getRegionSize: async (regionId: string) =>
    services.cleanupService.getRegionSize(regionId),
  ...
  setupAutoCleanup: async (
    options: RegionCleanupOptions & { intervalHours?: number } = {}
  ) => services.cleanupService.setupAutoCleanup(options),
  stopAutoCleanup: async (cleanupId?: string) =>
    services.cleanupService.stopAutoCleanup(cleanupId),
