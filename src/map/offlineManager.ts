...
import { downloadStyles, loadStyleById } from './styleManager';
import {
  downloadFonts,
...
export class OfflineMapManager {
  ...
  async addRegion(region: OfflineRegionOptions): Promise<void> {
    ...
    let style = await loadStyleById(styleResult.styleId);
    console.warn('Loaded style from DB:', style);

    // Patch style for offline use
    const patchedStyle = patchStyleForOffline(style as unknown as MapboxStyle, styleResult.styleId);
    ...
    await downloadTiles(region, patchedStyle, style!.id);

    // Save the updated style entry
    await db.put('styles', { ...styleEntry, key: style!.id });
  }
  ...
  async loadRegion(region: OfflineRegionOptions): Promise<void> {
    ...
    // Load tiles from storage (pass region, not styleEntry)
  ...
  private async getAllGlyphStats(): Promise<EnhancedGlyphStats> {
    ...
    for (const style of styles) {
      try {
        ...
        if (
          styleStats.oldestGlyph &&
          (!oldestGlyph || styleStats.oldestGlyph.timestamp < oldestGlyph.timestamp)
        ) {
          oldestGlyph = styleStats.oldestGlyph;
        }
        if (
          styleStats.newestGlyph &&
          (!newestGlyph || styleStats.newestGlyph.timestamp > newestGlyph.timestamp)
        ) {
  ...
  async performCompleteMaintenance(
    ...
    try {
      ...
      if (options.generateReport) {
        ...
        results.analyticsReport = {
          styles: 0, // Set to actual number of styles if available
          regions: analytics.regions,
          tiles: {
            count: analytics.tiles.count,
            totalSize: analytics.tiles.totalSize,
            byZoomLevel: Object.fromEntries(
              (analytics.tiles.zoomLevelStats ?? new Map()).entries()
            ),
          },
          fonts: {
            count: analytics.fonts.count,
            totalSize: analytics.fonts.totalSize,
            byType: analytics.fonts.fontsByType ?? {},
          },
          glyphs: {
            count: analytics.glyphs.count,
            totalSize: analytics.glyphs.totalSize,
          },
          sprites: {
            count: analytics.sprites.count,
            totalSize: analytics.sprites.totalSize,
            byType: analytics.sprites.spritesByType ?? {},
          },
        };
        currentProgress++;
      }
