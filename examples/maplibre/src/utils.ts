...
export const updatePolygons = (
  ...
  const clippedPolygonAreaKM2 = convertArea(
    clippedPolygonAreaM2,
    'meters',
    'kilometers',
  );
  setArea(parseFloat(clippedPolygonAreaKM2.toFixed(2)));

  const leftoverPolygon = difference(
    featureCollection([originalPolygon, clippedPolygon]),
  );
  ...
  if (map.current?.getSource('leftoverPolygon')) {
    (map.current.getSource('leftoverPolygon') as GeoJSONSource).setData(
      leftoverPolygon,
    );
  } else {
